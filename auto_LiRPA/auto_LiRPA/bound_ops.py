...
class BoundLinear(Bound):
        ...
        if not self.is_input_perturbed(1) and (not has_bias or not self.is_input_perturbed(2)):
            ...
            if isinstance(last_lA, eyeC) and isinstance(last_uA, eyeC):
                ...
                def _bound_oneside(last_A):
                    ...
                    next_A = last_A.matmul(input_lb[1])
                    sum_bias = last_A.matmul(input_lb[2]) if has_bias else 0.0
    ...
    def bound_backward_with_weight(self, last_lA, last_uA, input_lb, input_ub, x, y):
        ...
        self.x_shape = input_lb[0].size()
        self.y_shape = input_lb[1].size()
...
class BoundBatchNormalization(Bound):
    ...
    def interval_propagate(self, *v):
        ...
        if self.bn_mode == 'ibp' and not hasattr(self, 'fv'):
...
class BoundRelu(BoundActivation):
    def __init__(self, input_name, name, ori_name, attr, inputs, output_index, options, device):
        ...
        # If this is set to true, we can get gradients for beta. Used for heuristics.
        # self.get_beta_gradients = self.options.get('optimize_bound_args', {}).get('ob_get_heuristic', False)
        ...
        self.beta = None
        self.beta_mask = None
        self.masked_beta = None

    def clean_slope(self):
        self.slope = None

    def _init_slope(self, x):
        self.final_shape = self.options.get('final_shape')
    ...
    def forward(self, x):
        if self.relu_options == "random_evaluation":
            # if self.slope is None or self.slope.shape != x.shape:
            if self.slope is None:
                self._init_slope(x)

            if self.beta is None:
                # self.beta = torch.zeros([self.relu_used_count, *x.shape], dtype=torch.float).to(x.device)  # multiple beta
                self.beta = torch.zeros([1, *x.shape], dtype=torch.float).to(x.device)  # single beta
    ...
    def bound_backward(self, last_lA, last_uA, x=None):
        ...
        elif self.relu_options == "random_evaluation":
            self.relu_used_count -= 1
        ...
        def _bound_oneside(last_A, d_pos, d_neg, b_pos, b_neg):
            ...
            if type(last_A) == torch.Tensor:
                ...
                if last_A.identity == 0:
                    ...
                    d_pos_unfolded = d_pos_unfolded.view(d_pos_unfolded.size(0), d_pos_unfolded.size(1), last_A.patches.size(-3), last_A.patches.size(-2), last_A.patches.size(-1)).unsqueeze(2)
                    d_neg_unfolded = d_neg_unfolded.transpose(-2, -1)
                    d_neg_unfolded = d_neg_unfolded.view(d_pos_unfolded.size(0), d_pos_unfolded.size(1), last_A.patches.size(-3), last_A.patches.size(-2), last_A.patches.size(-1)).unsqueeze(2)
                    ...
                    prod, pos_A_patches, neg_A_patches = self.clamp_mutiply(last_A.patches, d_pos_unfolded, d_neg_unfolded)
        ...
        if self.beta is not None:
            # FIXME: we should never use XX.abs().sum() != 0 condition, this is a waste of time!
            if self.options.get('optimize_bound_args', {}).get('ob_get_heuristic', False) or self.beta_mask.abs().sum()!=0:
                # assert (self.beta[idx].shape == self.beta_mask.shape)
                self.masked_beta = (self.beta[0] * self.beta_mask).requires_grad_()
                if self.options.get('optimize_bound_args', {}).get('ob_get_heuristic', False):
                    self.masked_beta = self.masked_beta.requires_grad_()
                    # We retain gradients for masked_beta for branching heuristics.
                    self.masked_beta.retain_grad()
                ...
                if type(A) is Patches:
                    ...
                    if uA is not None:
                        uA = uA + self.masked_beta
                    if lA is not None:
                        lA = lA - self.masked_beta
                else:
